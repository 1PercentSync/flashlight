...
- 如果你认为 scope 或 file_types 范围外的代码对理解查询结果至关重要（如被引用的依赖、相关类型定义），可以一并返回，但优先返回范围内的结果
...
你正在查看的是项目的一个子集。项目被分成了多个分片并行查询，你只负责在你收到的文件中查找相关代码。对于任意一个查询，大多数分片不会包含相关代码——返回空结果是完全正常且期望的行为。不要为了"有所贡献"而降低相关性标准。
...
- 如果你认为 scope 或 file_types 范围外的代码对理解查询结果至关重要（如被引用的依赖、相关类型定义），可以一并返回，但优先返回范围内的结果
- 只返回你分片中实际包含的文件，不要返回你没有内容的文件
- 如果无法找到相关代码，调用 report_search_results({ "results": [] })`;

/** Get the static system instructions for single-shard mode. */
export function getSystemInstructions(): string {
  return SYSTEM_INSTRUCTIONS;
}

/** Get the static system instructions for multi-shard mode. */
export function getShardedSystemInstructions(): string {
  return SHARDED_SYSTEM_INSTRUCTIONS;
}

/** Build the base context message containing all files sorted by git time (oldest first). */
...
/** Build the base context for a single shard (subset of files). */
...
/** Build the incremental change context containing updated/deleted files since the base. */
...
/** Build the final query message with directory tree, scope, file type filters, and the user query. */
